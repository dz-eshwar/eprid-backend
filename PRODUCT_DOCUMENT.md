...
- Frontend: `/cpcb-directory` (sign-in gate if logged out) → `CpcbRecyclerSearchForm` (name/GST text inputs, state as a `<select>` populated from `/states`) + `CpcbRecyclerResultCard` list, each card showing an explicit risk-level `RiskPill` next to the numeric score (previously score-only) plus the score-direction note. Linked from `Navbar` — hidden for `RECYCLER` role (not relevant to a recycler checking on other recyclers). New `/admin/cpcb-review` page (`ADMIN`-only, gated client-side on `user.role`) lists the pending-review queue (each item's latest score + up to 20 recent field diffs, labeled via a `FIELD_LABELS` map) and recent refresh-run history with status coloring, plus a manual "refresh now" button; linked from `Navbar` as "CPCB Review" for `ADMIN` users.
