...
**Request correlation & logging** (added 2026-07-17, `66138ad`) — backend `RequestLoggingFilter` assigns/reuses an `X-Request-Id` per request (echoed on the response, `exposedHeaders` in CORS config), puts it plus `userId`/method/path in SLF4J MDC, and logs one summary line per request. `logback-spring.xml` renders human-readable console lines in dev and one JSON object per line (Logstash encoder) under the `prod` Spring profile for structured log search. Frontend's `apiFetch` (`lib/api/client.ts`) reads the response's `X-Request-Id` and throws a typed `ApiError` (status + requestId) instead of a plain `Error` on failed requests, logging the failure with its requestId to the browser console — lets a reported frontend error be matched to its exact backend log lines.

## 3. Modules

### Landing page (public marketing site)
Marketing homepage at `/` (hero, problem bar, waste-streams section, how-it-works, who-it's-for, now/next roadmap, footer). CTA links straight to `/register` — no waitlist gate (an earlier Resend-backed waitlist form was replaced with direct signup once the app went live; the `/api/waitlist` route handler is still present but no longer called from the page). Waste-streams section is a 3-card promo (Battery/Tyre → `/verify`, Used-Oil → `/used-oil`) added when Module E shipped. Landing nav and the authenticated app `Navbar` both got a direct `/used-oil` link at the same time. Fraud stat in the hero/problem bar corrected 2026-07-21 (`d213ea8`) from a "₹700Cr+" rupee figure to "600K–700K" certificates, matching the actual plastic-EPR fraud scale (a rupee amount was never sourced).

**Now/Next roadmap section** (`NowNextSection`, added 2026-07-21) — public, anchored `#now-and-next`, linked from both nav bars, the footer, and the "Verified Recycler" badge in the who-it's-for section (previously a static `<span>`, now a link into this section; copy changed to "Verified Recycler — Coming Soon" since the badge itself isn't built yet — see §7 for the reserved purple `#534AB7` badge color, distinct from the risk-band colors). Lists what's actually shipped ("Now": risk check, calculator, free directory, vault, used-oil assistant, tyre check) against what's being built next ("Next": Form 4 filing checklist, verified-recycler badge, deeper invoice-fraud checks, tyre-check expansion) — content lives in `messages/{en,hi,te}.json` under `landing.nowNext`, not wired to any backend state, so it only reflects reality if kept in sync by hand alongside actual ships.

**Google Analytics** (`components/analytics/GoogleAnalytics.tsx`, added 2026-07-21) — GA4 script tag, only rendered when `NEXT_PUBLIC_GA_MEASUREMENT_ID` is set **and** `NODE_ENV === "production"` (dev/preview traffic excluded by design), `anonymize_ip: true`. Mounted once in the locale root layout.

- Frontend: `components/landing/LandingPage.tsx` (`StreamsSection`, `NowNextSection`), `components/layout/Navbar.tsx`, `components/analytics/GoogleAnalytics.tsx`, locale-aware nav with language switcher
...
Report generator (`ReportService`, PDFBox) aggregates plausibility, forensics, regulatory history, and the composite score breakdown into one PDF. Free-text fields (LLM regulatory summaries, user input) are sanitized to WinAnsi-safe ASCII before rendering — PDFBox's Standard14 fonts throw on curly quotes/em-dashes/non-Latin characters otherwise. Disclaimer copy expanded 2026-07-15 (`4ee01c8`): each finding line now also prints a `Dated:` field (`findingDate`, or "not recorded" if null); a one-line note directly under the overall rating flags it as "a signal derived from available public/regulatory data, not a legal finding or a verdict"; and the closing disclaimer paragraph now also states findings aren't independently re-verified with the recycler, the recycler hasn't had a chance to respond to any specific finding, and source data may have changed since. The same disclaimer copy is now shown inline on the frontend's `CompositeScoreCard` too (below), not just the PDF.

**Evidence file storage** (`FileStorageService`) — fixed 2026-07-17 (`66138ad`): the stored filename's extension is now filtered to alphanumeric characters only (max 10 chars) before being appended to a fresh UUID, and the resolved target path is normalized and checked to still sit inside the per-check upload directory. Previously a crafted `originalFilename` (e.g. containing `../` segments) could escape that directory when building the stored `Path` — a path-traversal opening, now closed.
...
- Backend: `VaultController` (`/api/v1/vault/*`), soft-delete, ownership-checked download. `VaultService.listForRecycler` fixed 2026-07-17 (`66138ad`): previously returned a recycler's full doc list to any authenticated caller who knew that `recyclerId` (e.g. from an unrelated check), a cross-tenant IDOR — now scoped to the owning recycler, or (for anyone else) only documents that caller personally uploaded.
...
At RECYCLER registration, user optionally supplies GSTIN, legal business name, Udyam number, CIN/DIN, PAN (added 2026-07-15). Backend runs credential checks (GST/Udyam/MCA/PAN) through a `KycProvider` interface — a real vendor can drop in later behind the same interface with zero call-site changes. PAN verification only fires when both `pan` and `legalName` are supplied — it's a name-match check, not PAN-format validation alone. **Never blocks registration**: the check call is wrapped in try/catch and always swallowed on failure. No vendor is wired up yet — the stub provider (`app.kyc.provider=stub`, default) always returns `COULD_NOT_VERIFY` for every check type, PAN included. GST OTP verification exists on the interface but isn't called from `register()` (needs a two-step synchronous flow, PRD open item).
...
Added 2026-07-15: a **live refresh pipeline** (`CpcbRecyclerRefreshService`) sits alongside the manual-CSV ingestion path above — both funnel through a shared `CpcbRecyclerRowMapper` (extracted from the old `CpcbRecyclerIngestionService` so the upsert/data-quality-heuristic logic can't fork between the two paths). `CpcbRecyclerJsonFetcher` pulls the full live dataset directly from CPCB's own `eprbattery.cpcb.gov.in/user/recyclerview` endpoint — reverse-engineered as a Laravel DataTables endpoint: a plain GET returns the HTML page, but adding DataTables' own AJAX query params (`draw`/`start`/`length=-1`) plus an `X-Requested-With: XMLHttpRequest` header returns the full JSON dataset in one call (555/555 rows, no pagination loop), same field names as the CSV contract. Fails loudly (throws → refresh run marked `FAILED`) rather than silently mis-ingesting if CPCB ever changes the response shape.

Runs nightly at 4:30am IST (`@Scheduled(cron = "0 30 4 * * *")`) or on demand (`POST /api/v1/cpcb-recyclers/refresh`, `ADMIN` only). Each run is a **diff-and-flag** pass, not silent-overwrite: only 9 scoring-relevant fields are tracked (consent/HWMD/DIC expiries, capacity, type, inspection/internal-app status, certificate flag — cosmetic fields like phone/email/address don't count); a changed tracked field writes one `CpcbRecyclerSnapshotDiff` row (not one row per unchanged recycler) and triggers a rescore. If a rescore flips the recycler's risk **band** (not just the numeric score), the recycler is marked `pendingReview = true` and `CpcbRecyclerSearchService` holds back the just-flipped score from public search results — it keeps showing the previous score/band until an admin clears the flag (`POST /api/v1/cpcb-recyclers/{id}/confirm-review`). A previously-seen `cpcbId` absent from a pull is flagged `noLongerListedAt` (cleared automatically if it reappears) — never deleted, since CPCB-side pagination hiccups and real deregistration look identical from here. Every row also gets `lastSyncedAt` stamped, surfaced to the frontend as a "data as of" timestamp. Each row's insert/update runs inside its own `REQUIRES_NEW` transaction (added 2026-07-16, `b402d6e`) — a single bad row (constraint violation, malformed data) rolls back and is caught into the run's per-row error list, rather than marking the whole run's transaction rollback-only and cascade-failing every row processed after it. Each run is persisted as a `CpcbRefreshRun` (fetched/changed/new/missing counts, `RUNNING`/`SUCCESS`/`PARTIAL`/`FAILED` status, error detail) — history via `GET /api/v1/cpcb-recyclers/refresh-runs`.

- Backend: `CpcbRecyclerController`, `CpcbRecyclerIngestionService`, `CpcbRecyclerRowMapper`, `CpcbRecyclerCsvParser`, `CpcbRecyclerRefreshService`, `CpcbRecyclerJsonFetcher`, `CpcbRecyclerScoring`/`CpcbRecyclerScoringService`/`CpcbRecyclerScoringBackfillRunner`, `CpcbRecyclerSearchService`, `CpcbStateCodeRepository`, `CpcbRefreshRunRepository`, `CpcbRecyclerSnapshotDiffRepository`; migrations V12 (`cpcb_recyclers`/`cpcb_recycler_authorizations`/`cpcb_recycler_scores`/`cpcb_geo_risk_hotspots`) + V13 (seed data) + V14 (extra `CpcbRecycler` columns incl. ISO flags) + V15 (full 553-row directory seed, supersedes V13's sample) + V16 (`ENTITY_HEALTH`→`ENTITY_RISK` rename, data + column default) + V17 (`cpcb_state_codes`) + V21 (`last_synced_at`/`pending_review`/`no_longer_listed_at` columns, `cpcb_refresh_run` + `cpcb_recycler_snapshot_diff` tables); routes `POST /api/v1/cpcb-recyclers/ingest`, `GET /api/v1/cpcb-recyclers/search`, `GET /api/v1/cpcb-recyclers/states`, `POST /api/v1/cpcb-recyclers/refresh`, `GET /api/v1/cpcb-recyclers/refresh-runs`, `GET /api/v1/cpcb-recyclers/pending-review`, `POST /api/v1/cpcb-recyclers/{id}/confirm-review` (last 4 all `ADMIN` only)
- Frontend: `/cpcb-directory` (sign-in gate if logged out) → `CpcbRecyclerSearchForm` (name/GST text inputs, state as a `<select>` populated from `/states`) + `CpcbRecyclerResultCard` list, each card showing an explicit risk-level `RiskPill` next to the numeric score (previously score-only) plus the score-direction note and (added 2026-07-21, `d213ea8`) a "data as of `<date>`" line rendered from `result.lastSyncedAt` when present — resolves the earlier gap where `lastSyncedAt` was plumbed through the API layer but not shown on the public search card, only on the admin review page. Linked from `Navbar` — hidden for `RECYCLER` role (not relevant to a recycler checking on other recyclers). New `/admin/cpcb-review` page (`ADMIN`-only, gated client-side on `user.role`) lists the pending-review queue (each item's latest score + up to 20 recent field diffs, labeled via a `FIELD_LABELS` map) and recent refresh-run history with status coloring, plus a manual "refresh now" button; linked from `Navbar` as "CPCB Review" for `ADMIN` users.
...
| `RecyclerCredentialCheck` | Module A0 — one row per KYC check attempt (GST/Udyam/MCA/PAN), result history preserved |
...
| `CpcbRefreshRun` | Module F — one row per scheduled/manual live-refresh pull, added 2026-07-15 (fetched/changed/new/missing counts, RUNNING/SUCCESS/PARTIAL/FAILED status, error detail) |
| `CpcbRecyclerSnapshotDiff` | Module F — one row per changed tracked field per recycler per refresh run, added 2026-07-15 (no row for unchanged recyclers or untracked/cosmetic fields) |
...
`CpcbRecycler` gained 3 fields with the 2026-07-15 refresh pipeline: `lastSyncedAt` (nullable, set by every refresh pull that includes the row, null for CSV-only rows), `pendingReview` (boolean, default false, set on a risk-band flip), `noLongerListedAt` (nullable, set when a previously-seen `cpcbId` is absent from a pull, auto-cleared if it reappears).
...
- Module F refresh pipeline (added 2026-07-15): the CPCB `recyclerview` JSON endpoint contract is reverse-engineered (undocumented Laravel/DataTables behavior, not a published API) — the class-doc claims it was verified live once (2026-07-14, 555/555 rows) but `application.yml`'s own comment on the same commit still calls the URL/shape "not independently verified against the live endpoint this session," an inconsistency worth resolving rather than trusting either claim blindly; a future CPCB-side response-shape change fails the run loudly (good) but nothing currently alerts a human when that happens beyond checking `/refresh-runs` manually — no failure notification wired up. The `pendingReview` band-flip gate has no admin-facing badge/count/notification outside the dedicated `/admin/cpcb-review` page and no SLA on how long a flagged row can sit unreviewed while still being held back from public search. Two upsert paths (manual CSV `ingest`, live `refresh`) now share row-mapping logic via `CpcbRecyclerRowMapper` but are still two separate triggers with no reconciliation if they're run close together or disagree.
